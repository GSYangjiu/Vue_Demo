...
<script>
    ...
    import {getUserCount, getOrderCount, adminCount} from '@/api/getData'
    ...
    export default {
        ...
        methods: {
            async initData() {
                ...
                Promise.all([getUserCount(today), getOrderCount(today), adminCount(today), getUserCount(), getOrderCount(), adminCount()])
                    .then(res => {
                        this.userCount = res[0].map.count;
                        this.orderCount = res[1].map.count;
                        this.adminCount = res[2].map.count;
                        this.allUserCount = res[3].map.count;
                        this.allOrderCount = res[4].map.count;
                        this.allAdminCount = res[5].map.count;
                    }).catch(err => {
                    console.log(err)
                })
            },
            async getSevenData() {
                ...
                Promise.all(promiseArr).then(res => {
                    ...
                    res.forEach((item, index) => {
                        if (item.status == 10000) {
                            resArr[Math.floor(index / 7)].push(item.map.count)
                        }
                    })
